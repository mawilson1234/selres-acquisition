...
class SelectionalRestrictionEvaluator:
	...
	def __format_dataset(self, dataset: Union[Dict, DictConfig] = None) -> Dict:
		...
		'''
		gf_regex = re.sub(r'(\[|\])', '\\ \\1', '|'.join([arg for arg in self.cfg.data.args])).replace(' ', '')
		masked_args	= [re.findall(rf'({gf_regex})', sentence) for sentence in dataset]
		for i, _ in enumerate(dataset):
			for gf in list(self.cfg.data.args.keys()):
	...
	def __add_hyperparameters_to_summary_df(self, df: pd.DataFrame) -> pd.DataFrame:
		...
		'''
		exclude = [
			'mask_token', 'mask_token_id', 'unk_token_id', 'device',
		]
	...
	def __collect_results(
		...
		tokens_to_type_labels = {token: label for label in self.cfg.data.args for token in self.cfg.data.args[label]}
		if len(tokens_to_type_labels.keys()) < len(self.cfg.data.args.values()):
			raise ValueError('Tokens should only be used in a single eval group!')
		
		results = []
		metrics = tuple(zip(masked_indices, sentences, *get_output_metrics(outputs)))
		
		for sentence_num, (position_indices, sentence, logits, probs, logprobs, surprisals, predicted_ids) in enumerate(metrics):
			...
			for position in position_indices:
				for token, label in tokens_to_type_labels.items():
					...
					entropy 	= Categorical(probs=logprobs[position_indices[position]]).entropy()
	...
	def evaluate(self) -> None:
		...
		summary.to_csv(f'{file_prefix}-odds_ratios.csv.gz', index=False, na_rep='NaN')
		
		if self.cfg.create_plots:
			log.info('Creating odds ratios plots')
			self.create_plots(summary)
	...
	def get_summary(self) -> pd.DataFrame:
		...
		with logging_redirect_tqdm():
			for string_id in tqdm(self.all_checkpoints):
				...
				checkpoint_results	= self.__collect_results(outputs=outputs, sentences=sentences, masked_indices=masked_indices)
		...
		for token in summary.token.unique():
		 	summary.loc[summary.token == token, 'sentence type'] 	= selres_utils.flatten([np.repeat(st, len(self.cfg.data.args.keys())).tolist() for st in sentence_types]) * len(self.all_checkpoints)
		 	summary.loc[summary.token == token, 'sentence num']		= selres_utils.flatten([np.repeat(sn, len(self.cfg.data.args.keys())).tolist() for sn in sentence_nums]) * len(self.all_checkpoints)
	...
	def create_plots(self, *args: Tuple, **kwargs: Dict) -> None:
		
		'''
		Calls selres_plots.create_odds_ratios_plots
		
			params:
				*args (tuple)	: passed to selres_plots.create_plots
				**kwargs (dict)	: passed to selres_plots.create_plots
		'''
		selres_plots.create_plots(*args, **kwargs)
